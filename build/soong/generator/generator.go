...
func init() {
	android.RegisterModuleType("zap_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = zapExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = zapExpandVariables(ctx, dep_file)
	...
	cmd := zapExpandVariables(ctx, String(g.properties.Cmd))
